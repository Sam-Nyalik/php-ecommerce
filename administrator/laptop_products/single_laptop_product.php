...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["productName"]))){
        $productName_error = "Product Name is required!";
    } else {
    ...
    if(empty(trim($_POST["productDescription"]))){
        $productDescription_error = "Product Description is required!";
    } else {
    ...
    if(empty($_POST["productType"])){
        $productType_error = "Product Type is required!";
    } else {
    ...
    if(empty($_POST["productBrand"])){
        $productBrand_error = "Product Brand is required!";
    } else {
    ...
    if(empty(trim($_POST["productRetailPrice"]))){
        $productRetailPrice_error = "Product Retail Price is required!";
    } else {
    ...
    if(empty(trim($_POST["productPrice"]))){
        $productPrice_error = "Product Price is required!";
    } else {
    ...
    if(empty(trim($_POST["productQuantity"]))){
        $productQuantity_error = "Product Quantity is required!";
    } else {
    ...
    if(empty($productName_error) && empty($productDescription_error) && empty($productType_error) && empty($productBrand_error) && empty($productRetailPrice_error) && empty($productPrice_error) && empty($productQuantity_error)){
        ...
        if(!empty($_FILES["productImage"]["name"])){
            ...
            // Prepare an UPDATE statement to update the product on both client side and the server side
            $id = false;
            if(isset($_GET['id'])){
                $id = $_GET['id'];
            }

            $sql = "UPDATE all_products SET productName = :productName, productDescription = :productDescription, productType = :productType, productBrand = :productBrand, productRetailPrice = :productRetailPrice, productPrice = :productPrice, productQuantity = :productQuantity, productImage = :productImage WHERE id = '$id'";
            ...
            $productImage_error = "Product Image is required!";
        }
    }
}
...
<!-- Header Template -->
<?= headerTemplate('ADMIN | SINGLE_LAPTOP_PRODUCT'); ?>

<!-- Main Navbar -->
<?php include_once "./administrator/includes/main_navbar.php"; ?>

<!-- Prepare a SELECT statement to fetch the product from the database using the id in the url -->
<?php
$id = false;
if (isset($_GET['id'])) {
    $id = $_GET['id'];
}
$sql = $pdo->prepare("SELECT * FROM all_products WHERE id = '$id'");
$sql->execute();
$database_single_laptop_product = $sql->fetchAll(PDO::FETCH_ASSOC);
?>

<?php foreach ($database_single_laptop_product as $single_laptop_product) : ?>
    <!-- Section Title -->
    <div class="section-title" style="margin-top: 30px;">
        <div class="container">
            <div class="row">
                <h5>Edit <?= $single_laptop_product['productName']; ?></h5>
            </div>
        </div>
    </div>
    ...
    <div id="individual_product">
        <div class="container">
            <div class="row">
                <div class="col-md-6">
                    <img src="<?= $single_laptop_product['productImage']; ?>" alt="<?= $single_laptop_product['productName']; ?>" class="img-card-top img-fluid">
                </div>
                <div class="col-md-6">
                    <form action="#" method="post" enctype="multipart/form-data" class="individual_product_form">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="productName" value="<?= $single_laptop_product['productName']; ?>" class="form-control 
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="productDescription" class="form-control w-100 <?php echo (!empty($productDescription_error)) ? 'is-ivalid' : ''; ?>"><?= $single_laptop_product['productDescription']; ?></textarea>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="productType" class="form-control">
                                <option value="" disabled>Select a Product Type
                                    <!-- Prepare a SELECT statement to fetch product types from the database -->
                                    <?php
                        ...
                        <div class="form-group">
                            ...
                            <select name="productBrand" class="form-control">
                                <option value="" disabled>Select a Product Brand
                                    ...
                                    ?>
                                <option value="<?= $single_laptop_product['productBrand']; ?>" selected><?= $single_laptop_product['productBrand']; ?></option>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="productRetailPrice" class="form-control <?php echo (!empty($productRetailPrice_error)) ? 'is-invalid' : ''; ?>" value="<?= $single_laptop_product['productRetailPrice']; ?>">
                        </div>

                        <!-- Product Price -->
                        <div class="form-group">
                            <label for="productPrice">Product Price</label>
                            <input type="number" name="productPrice" class="form-control <?php echo (!empty($productPrice_error)) ? 'is-invalid' : ''; ?>" value="<?= $single_laptop_product['productPrice']; ?>">
                        </div>

                        <!-- Product Quantity -->
                        <div class="form-group">
                            <label for="productQuantity">Product Quantity</label>
                            <input type="number" name="productQuantity" class="form-control <?php echo (!empty($productQuantity_error)) ? 'is-invalid' : ''; ?>" value="<?= $single_laptop_product['productQuantity']; ?>">
                        </div>

                        <!-- Product Image -->
                        <div class="form-group">
                            <label for="productImage">Product Image</label>
                            <input type="file" name="productImage" accept=".png, .jpg, .jpeg" class="form-control <?php echo (!empty($productImage_error)) ? 'is-invalid' : ''; ?>">
                        </div>
